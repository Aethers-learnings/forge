...
- [ ] T-104 — Define ownership and authorization rules for network, suggestions, endorsements, and conversations; introduce a migration plan before code changes. Depends on: T-101. Classification: MAJOR_REVIEW. Issue #9 design package prepared in [OWNERSHIP_DESIGN](OWNERSHIP_DESIGN.md), with proposed D-012–D-015; awaiting human architecture review. Ownership implementation is not complete or authorized.
...
- [ ] T-201 — Add reversible schema migration tooling and explicit indexes/constraints appropriate to verified access patterns. Depends on: human approval of the T-104 data design, including FK/admin-removal impact and legacy quarantine. Classification: MAJOR_REVIEW. Issue #9 supplies proposed stages/rollback/invariants only; no migration was created or run.
...
- [ ] T-203 — Incrementally extract Flask blueprints/services around identity, workflows, media, and analytics. Depends on: T-101, T-202. Classification: SAFE_INCREMENTAL. First increment (Issue #1, PR #2): notification/onboarding blueprints merged. Second increment (Issue #3, PR #4): five profile workflow routes extracted into `forge_routes/profile.py` and merged after 50 new baseline regressions; full suite 256 passed. Third increment (Issue #5, 2026-09-26): three analytics handlers extracted into `forge_routes/analytics.py` after 28 new baseline regressions; full suite 285 passed, URL/schema/AST comparisons pass, pending PR review. Image/storage and public-profile routes remain in the entrypoint under D-010. Remaining identity, workflow, and media extraction stays open; do not combine it with T-104/T-201 ownership/schema changes.
