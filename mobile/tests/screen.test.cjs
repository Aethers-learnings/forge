...
function screen(saved, options = {}) {
  ...
  const modules = {
    ...
    '../api/client': {
      ...
      updateProfilePortfolio: async (_url, fields) => ({
        id: 1,
        username: 'account',
        name: 'Account',
        role: 'trade',
      ...
      addPostComment: async (_url, id, text) => ({
        id,
        name: 'Account',
        role: 'trade',
        body: 'Post',
